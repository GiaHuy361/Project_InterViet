...
export const AdminSystemHealthPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <Card className="p-12 text-center text-gray-500 dark:text-slate-400">Đang tải trạng thái hệ thống...</Card>
      ) : !health ? (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-gray-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400">Cập nhật lúc: {new Date(health.timestamp).toLocaleString('vi-VN')}</p>
              </div>
            </div>
            <div className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-bold border-2 ${
              health.status === 'Healthy' ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800' :
              health.status === 'Degraded' ? 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800' :
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            {Object.entries(health.components).map(([key, comp]) => (
              <div key={key} className={`p-6 border rounded-xl border-l-8 shadow-sm flex items-start gap-4 transition-all hover:shadow-md ${
                comp.status === 'Healthy' ? 'border-l-green-500 bg-white dark:bg-slate-900 border-gray-100' :
                comp.status === 'Degraded' ? 'border-l-yellow-500 bg-yellow-50 dark:bg-yellow-900/30 border-yellow-100 dark:border-yellow-800/50' :
                'border-l-red-500 bg-red-50 dark:bg-red-900/30 border-red-100 dark:border-red-800/50'
              }`}>
                ...
                <div className="flex-1">
                  <h3 className="font-bold capitalize text-lg flex items-center justify-between gap-2 mb-2">
                    ...
                    <span className={`text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full font-bold ${
                      comp.status === 'Healthy' ? 'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-300' :
                      comp.status === 'Degraded' ? 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-800 dark:text-yellow-300' :
                      'bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300'
                    }`}>
                      {comp.status}
                    </span>
                  </h3>
                  <p className="text-gray-600 dark:text-slate-400 text-sm leading-relaxed">{comp.details}</p>
                </div>
              </div>
            ))}
          </div>
