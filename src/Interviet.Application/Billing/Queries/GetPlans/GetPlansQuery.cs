    ...
    public async Task<Result<List<PlanResponse>>> Handle(GetPlansQuery request, CancellationToken ct)
    {
        ...
        var responses = plans.Select(p =>
        {
            return new PlanResponse
            {
                ...
                DisplayPrice = p.Code == "quarterly" ? 166333 : (p.Code == "yearly" ? 249916 : null),
                Badge = p.Code == "quarterly" ? "Phổ biến nhất" : (p.Code == "yearly" ? "Tiết kiệm nhất" : (p.Code == "combo" ? "Mua thêm lượt" : null)),
